...
def Generate_Stations():
    ...
    while counter <= number_of_stations-1:
        ...
        counter += 1
    Target_station_pos.append(rnd.choice(Station_Locatons))
...
def Trains(Station_Locatons, Station_Count):
    ...
    def reschedual(Trainx, Trainy, count):
        ...
        for target in Targets:
            if sqrt((target[0]-Target_station_pos[0][0])**2 + (target[1]-Target_station_pos[0][1])**2) <= distance:
                best_target = count2
                distance = sqrt((target[0]-Target_station_pos[0][0])**2 + (target[1]-Target_station_pos[0][1])**2)
            count2 += 1
        Train_Targets[count] = Targets[best_target]
